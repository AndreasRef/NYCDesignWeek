...
//EMULATE the current test setup, so you split the strips into their sides to emulate the final setup!
...
void draw() {
  ...
  if (testObserver.hasStrips) {
    ...
    if (waveShow == false) {
      for (int i = 0; i<stripNumbers*sidesPerStrip; i++) {
        if (triggerValue == i && vibrationTrigged) { //If a light vibration sensor has recently been trigged
          ...
          //Attempting to use the side function istead
          if (i<lights.length) pushSide(floor(i/3), i%3, color(lights[i].fillC)); //This works!
  ...
  int selectedSide = (int)map(mouseX, 0, width, 0, 18);

  for (int i = 0; i < 18; i++) {

    //pushSide(floor(i/3), i%3, color(#000000));
    if (selectedSide == i) {
      //pushSide(floor(selectedSide/3), selectedSide%3, color(#00FF00));
    } 
  }
}
